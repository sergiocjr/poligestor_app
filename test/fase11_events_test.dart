...
import 'package:poligestor_app/features/events/data/events_contracts.dart';
...
void main() {
  group('AuthMode Fase 11 events paths', () {
    test('exposes official /v1/events namespace', () {
      ...
      expect(m.eventsListPath, '/v1/events/events');
      ...
      expect(m.eventsAudiencesPath, '/v1/events/hearings');
      expect(m.eventsMeetingsPath, '/v1/events/meetings');
      expect(m.eventsParticipantsPath, '/v1/events/participants');
      expect(m.eventsInvitesPath, '/v1/events/invitations');
      expect(m.eventsAttendancePath, '/v1/events/attendance');
      expect(m.eventsCheckInPath, '/v1/events/attendance');
      expect(m.eventsCheckOutPath, '/v1/events/attendance');
      expect(m.eventsQrCodePath, '/v1/events/certificates');
      expect(m.eventsGalleryPath, '/v1/events/gallery');
      expect(m.eventsPhotosPath, '/v1/events/gallery');
      expect(m.eventsVideosPath, '/v1/events/gallery');
      ...
      expect(m.eventsIndicatorsPath, '/v1/events/statistics');
      expect(m.eventsSearchPath, '/v1/events/events');
      expect(m.eventsMapPath, '/v1/events/agenda');
    });
  });

  group('Events LIVE contracts', () {
    test('matches catalog cards with assumed AuthMode remaps', () {
      expect(kEventsLiveSlugs.length, 23);
      expect(eventsPathLive('list'), isTrue);
      expect(eventsPathLive('events'), isTrue);
      expect(eventsPathLive('audiences'), isTrue);
      expect(eventsPathLive('invites'), isTrue);
      expect(eventsPathLive('indicators'), isTrue);
      expect(eventsPathLive('check-in'), isTrue);
      expect(eventsPathLive('viewer'), isFalse);
    });
  });
